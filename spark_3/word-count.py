...
def word_count(input_path, output_path):
    ...
    if input_path is not None:
        ...
        #fs = FileSource.for_record_stream_format(StreamFormat.text_line_format(), "data_flink/")\
            #.process_static_file_set().build()
        fs = FileSource.for_record_stream_format(StreamFormat.text_line_format(), "data_flink/") \
            .monitor_continuously(Duration.of_seconds(15)).build()
    ...
    # compute word count
    ds = ds.flat_map(lambda s: s.lower().split()) \
        .filter(lambda i: re.match(r'[a-z]+', i)) \
